...
def print_intermediate_search_result(search_results: List[SLPPD], done_files: int, elapsed_time: float):
    if search_results:
        ...
        print("%s[%d done, %.2f docs/s] cur top-1: %.4f %d %s:%d-%d" % (ANSI_ESCAPE_CLEAR_CUR_LINE, done_files, done_files / elapsed_time, sim, para_len, df, pos[0] + 1, pos[1]), end="", file=sys.stderr, flush=True)
